...
// Similar to 02_bad but using longer fully qualified name/less specific imports
fn encrypt(plaintext: &[u8]) -> Result<Vec<u8>, age::EncryptError> {
    // Encrypt the plaintext to a ciphertext using the passphrase...
    let encrypted = {
        //  HardcodedSecret VULNERABILITY HERE
        let encryptor = age::Encryptor::with_user_passphrase(age::secrecy::Secret::new("this is not a good passphrase".to_owned()));
...
fn decrypt(ciphertext: Vec<u8>) -> Result<Vec<u8>, age::DecryptError> {
    ...
    //  HardcodedSecret VULNERABILITY HERE
    let mut reader = decryptor.decrypt(&age::secrecy::Secret::new("this is not a good passphrase".to_string()), None)?;
